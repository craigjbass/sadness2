...
The `home-routes` handler in the `sadness2.routes.home` namespace
...
The `home-page` function will in turn call the `sadness2.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `sadness2.handler` namespace:
...
Request middleware functions are located under the `sadness2.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `sadness2.dev-middleware` namespace found in
...
* Let `mount` know to start the database connection by `require`-ing `sadness2.db.core` in some other namespace.
